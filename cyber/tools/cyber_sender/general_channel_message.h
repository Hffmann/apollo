...
#ifndef TOOLS_CVT_SENDER_GENERAL_CHANNEL_MESSAGE_H_
#define TOOLS_CVT_SENDER_GENERAL_CHANNEL_MESSAGE_H_
...
// //           sink_ = sink;
          ...
          // Socket with direct access to the sink: used to synchronize start of batch
          sink_ = zmq::socket_t(context_, ZMQ_PUSH);
          sink_.connect("tcp://localhost:5559");
...
//           HasInitializedSocket();
  ...
  void HasInitializedSocket(void) {
    initialized_socket_ = true;
  }

  GeneralChannelMessage* OpenChannel(const std::string& channel_name);

  void SendString(std::string sender_string);
