...
#include "cyber/tools/cyber_sender/general_message_base.h"

#include <iomanip>
#include <string>
#include <vector>

#include "cyber/tools/cyber_sender/general_channel_message.h"
#include "cyber/tools/cyber_sender/general_message.h"
#include "cyber/tools/cyber_sender/screen.h"
...
void GeneralMessageBase::SendMessage(GeneralMessageBase* baseMsg,
...
//     }
    ...
    if (fieldName == "trajectory_point" || fieldName == "header") {
      baseMsg->isSenderField();
      baseMsg->setNextFieldName(fields[i + 1]->name(), true);
    } else if (fieldName == baseMsg->getNextFieldName() && baseMsg->getIsSenderField()) {
      baseMsg->isSenderField();
      baseMsg->setNextFieldName("", true);
    } else if (fieldName == "z" && baseMsg->getIsSenderField()) {
      baseMsg->isSenderField();
      baseMsg->setNextFieldName(fields[i + 1]->name(), false);
    } else if (fieldName == baseMsg->getNextFieldName() && !baseMsg->getIsSenderField()) {
      baseMsg->isSenderField();
      baseMsg->setNextFieldName();
    }
//     baseMsg->insertString(fieldName);
...
//         baseMsg->insertString(std::to_string(reflection->FieldSize(msg, field)));

        std::ostringstream out_str;

        out_str << fieldName << ": ";

        for (auto j = 0; j < reflection->FieldSize(msg, field); ++j) {
...
//             }
          if (field->cpp_type() == google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE){
            SendMessage(baseMsg, reflection->GetRepeatedMessage(msg, field, j),
                jump_lines, s, line_no, indent + 2);
          }
          else {
            SendField(baseMsg, msg, jump_lines, s, line_no, indent + 2, reflection, field, j);
          }
        }
      }
    } else {
      SendField(baseMsg, msg, jump_lines, s, line_no, indent, reflection,
                 field, -1);
    }  // end else
  }    // end for
...
void GeneralMessageBase::SendField(
...
#define OUTPUT_FIELD(CPPTYPE, METHOD, PRECISION)                    \
  case google::protobuf::FieldDescriptor::CPPTYPE_##CPPTYPE:        \
    if (*jump_lines) {                                              \
      ...
      sender_stream << std::fixed << std::setprecision(PRECISION)   \
              << (field->is_repeated()                              \
                      ? ref->GetRepeated##METHOD(msg, field, index) \
                      : ref->Get##METHOD(msg, field));              \
      out_str << baseMsg->insertString(sender_stream.str());        \
...
#undef OUTPUT_FIELD
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_STRING: {
      ...
      const std::string& str =
          baseMsg->insertString(field->is_repeated()
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_ENUM: {
      if (*jump_lines) {
        ...
        if (enum_desc != nullptr) {
          ...
          out_str << baseMsg->insertString(std::to_string(field->is_repeated()
                             ? ref->GetRepeatedEnumValue(msg, field, index)
                             : ref->GetEnumValue(msg, field)));
        }
        s->AddStr(indent, (*line_no)++, out_str.str().c_str());
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
      ...
      GeneralMessageBase::SendMessage(
          baseMsg,
