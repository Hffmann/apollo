...
// void SendData(zmq::socket_t& socket, const std::string& string) {
...
void GeneralChannelMessage::SendString(std::string sender_string) {

  if (!initialized_socket_){
    zmq::message_t sink_message(2);
    std::memcpy(sink_message.data(), "0", 1);
    sink_.send(sink_message);

    HasInitializedSocket();
  }

  zmq::message_t message(sender_string.size());
  std::memcpy(message.data(), sender_string.data(), sender_string.size());
    ...
    if (raw_msg_class_ == nullptr) {
      ...
      if (channel_msg->message.size()) {
        ...
        if (raw_msg_class_->ParseFromString(channel_msg->message)) {
          ...
          jump_lines <<= 2;
          jump_lines /= 5;
          GeneralMessageBase::SendMessage(this, *raw_msg_class_, &jump_lines,
                                           s, line_no, 0);

//           std::cout << "\n\n\n\n" << channel_reader_->GetChannelName().c_str() << "\n\n\n\n" << std::endl;

          SendString(this->getStream().str());
