...
//    Prepare our context and socket
  ...
  receiver.setsockopt(ZMQ_CONFLATE, 1);
...
//     std::cout << "\n" << rpl << "\n" << std::endl;
    ...
    producer_->Start(std::string(static_cast<char*>(message.data())));
//     producer_->Start(message);
/*
...
//     consumer_->Start(play_param.begin_time_ns);
    consumer_->Start(Time::Now().ToNanosecond());
...
//     std::cout << "\rPASSED ! ";
    ...
    while (!is_stopped_.load() && apollo::cyber::OK()) {
      ...
      if (is_paused_) {
        consumer_->Pause();
        std::cout << "\r[PAUSED] Time: ";
      } else {
        consumer_->Continue();
        std::cout << "\r[LIVE] Time: ";
      }
